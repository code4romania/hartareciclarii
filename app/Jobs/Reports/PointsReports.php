...
class PointsReports implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        $points = Point::query()
            ...
            ->when(filled($structure['service_ids']), fn (Builder $query) => $query->whereIn('service_type_id', $structure['service_ids']))
            ->when(filled($structure['point_type_ids']), fn (Builder $query) => $query->whereIn('point_type_id', $structure['point_type_ids']))
//            ->when(filled($structure['material_ids']), fn (Builder $query) => $query->whereHas('materials', fn (Builder $query) => $query->whereIn('material_id', $structure['material_ids'])))
        ...
        $names = match ($group_by) {
            ...
            'status' => [
                'active' => __('report.status.active'),
                'inactive' => __('report.status.inactive'),
                'pending' => __('report.status.pending'),
                'rejected' => __('report.status.rejected'),
                'approved' => __('report.status.approved'),
            ],
        };
        $data = [];
