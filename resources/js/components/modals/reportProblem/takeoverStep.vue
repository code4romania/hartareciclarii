	...
	<div class="">
		<div class="flex items-center justify-between">
			<h3 id="modal-title" class="text-2xl pl-5 py-3">{{
					CONSTANTS.LABELS.REPORT_PROBLEM.TAKEOVER_STEP.TITLE
				}}</h3>
		...
		<div
			...
			v-if="items.length > 0"
		>
			<span class="pt-10 pb-3 text-sm px-5 w-full">{{ CONSTANTS.LABELS.REPORT_PROBLEM.TAKEOVER_STEP.SUBTITLE }}</span>
			<fieldset>
					...
					>
						...
						>
							<input
								...
								v-model="stepRequestBody.collection_decline_reason"
								:value="item.id"
							/>
						</div>
						<div class="ml-3">
							<label :for="`comments_${item.id}`" class="text-gray-700 text-sm font-normal">{{item.title}}</label>
						</div>
					</div>
				</div>
			</fieldset>
		...
		<div class="flex items-center justify-between">
			<span class="pt-10 text-sm px-5">{{ CONSTANTS.LABELS.REPORT_PROBLEM.TAKEOVER_STEP.CAPTION }}</span>
		</div>
					...
					<div class="flex">
						...
						<div class="ml-3">
							<h3 class="text-sm font-medium text-red-800">
								{{ CONSTANTS.LABELS.REPORT_PROBLEM.TAKEOVER_STEP.DESCRIPTION_NOT_FOUND }}</h3>
						</div>
					</div>
				</div>
			</template>
