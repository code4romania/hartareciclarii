    ...
    >
        <div class="flex justify-between px-4 py-2 md:px-6 md:py-4 md:gap-6">
            ...
            <div class="hidden md:flex md:gap-4 md:items-center">
                ...
                <!-- Profile dropdown -->
                <Menu v-if="$page.props.auth" as="div" class="relative ml-3">
        ...
        <DisclosurePanel
            class="absolute inset-x-0 bg-white border-b border-gray-200 shadow md:hidden top-full"
            v-slot="{ close }"
        >
            <div class="px-2 pt-2 pb-3 sm:px-3" v-click-away="close">
                <div class="pt-2 pb-3 space-y-1">
                    <DisclosureButton :as="AddPoint" simple />
                    <DisclosureButton
                        v-for="(item, index) in navigation"
                        :key="index"
                        :as="Button"
                ...
                <div v-if="$page.props.auth" class="pt-4 pb-3 border-t border-gray-200">
                    <div class="flex items-center px-4">
                        ...
                        <div class="ml-3">
                            <div class="text-base font-medium text-gray-800">{{ $page.props.auth.name }}</div>
...
<script setup>
    ...
    const userNavigation = [
        { name: trans('profile.my_profile'), href: route('front.account.dashboard') },
        { name: trans('profile.settings'), href: route('front.account.settings') },
        { name: trans('profile.logout'), href: route('auth.logout'), method: 'post' },
    ];
