<template>
    <div class="grid gap-4">
        ...
        <ol
            class="w-full pt-10 -mt-10 overflow-hidden bg-white border border-gray-300 shadow rounded-2xl"
        ...
        <div class="h-full min-h-56">
            <div
                class="absolute z-10 w-full max-w-sm border-gray-100 rounded-lg bg-opacity-70 bg-gray-50 dark:bg-gray-800 dark:border-gray-800 dark:hover:bg-gray-700 h-1/3"
                v-if="loader"
            >
            ...
            >
                ...
                />
                <LMarker v-if="point" :lat-lng="{ lat: point.lat, lng: point.lng }" draggable @dragend="dragend" />
            </LMap>
        </div>
    </div>
</template>
...
<script setup>
    ...
    const runQuery = useDebounceFn(
        () => {
            axios
                .get(`https://nominatim.openstreetmap.org/search?format=json&q=${query.value}&addressdetails=1&limit=5`)
                .then((response) => {
